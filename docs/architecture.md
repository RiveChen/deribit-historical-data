...
- **Failure supervision**: the main coroutine watches the storage consumer during both initial task distribution and steady-state execution. A disk/DB flush error cancels producers and propagates to the caller instead of leaving full queues hanging. Tasks that exhaust their retry budget are drained into `dead_letters`, then `engine.run()` raises a summary error; callers cannot finalize or report success after partial fetch failure.
