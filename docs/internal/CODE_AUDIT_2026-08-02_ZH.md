...
> **工程门禁修复状态：已修复。** Ruff 已从运行时依赖移动到 dev group；覆盖率门禁由 0 提升到 80%（当前 83.96%）；CI 新增 `ruff format --check` 与 Pyrefly。与 CI 相同的命令在本地实测为 142 passed、Ruff 通过、Pyrefly 0 errors。旧 `TECH_DEBT` 与学习型 `ROADMAP` 已标注为历史材料，避免再被误读为当前状态。
...
- [x] 完整的 68113 行已过期 BTC future 与 10593 行已过期 BTC option 均完成 JSONL/Parquet 唯一键集合对账；
- [x] 校验能识别缺首段、缺尾段、缺 instrument 和重复掩盖缺口，并区分未知最终边界；
- [x] 任一读取/写入/checkpoint 错误导致非零退出，旧产物不被覆盖；
- [x] 压力测试证明小容量队列不会死锁，consumer 失败会在固定超时内向上冒泡；
- [x] CI 强制 test、lint、format、类型检查，并设置 80% 覆盖率下限；
- [x] README 已移除未经全量验收的“每一笔/固定耗时/无 OOM”承诺，`--no-dedup` 与大文件两种 reader 路径按实现在文档中明确。
